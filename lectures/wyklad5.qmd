...
description: "Apache Spark jako silnik rozproszonego przetwarzania, Structured Streaming, integracja Spark–Kafka, kompletny pipeline real-time analytics."
format:
  html:
    code-fold: true
    code-tools: true
...
## {{< fa memory >}} Przetwarzanie w pamięci
Dane trzymane w RAM, nie na dysku. Nawet **100x szybciej** niż MapReduce.
:::
::: {.callout-tip appearance="simple"}
## {{< fa cubes >}} Jeden silnik, wiele trybów
...
::: {.column width="50%"}
::: {.callout-tip appearance="simple"}
## {{< fa python >}} API w wielu językach
...
#### {{< fa wand-magic-sparkles >}} Transformacje (lazy)
...
## {{< fa database >}} Spark SQL — alternatywa
...
::: {.callout-caution}
## Projektując pipeline real-time, rozważ wymiar obliczeniowy
...
: Plan laboratoriów {.striped .hover}
...
::: {.callout-important appearance="simple"}
## {{< fa flag-checkered >}} To był ostatni wykład
...
::: {.callout-tip appearance="simple"}
## {{< fa laptop-code >}} Przed Lab 1
Upewnij się, że masz zainstalowanego Dockera i Gita (patrz strona [Narzędzia](info.qmd)). Sklonuj repozytorium kursu i uruchom `docker compose up` — jeśli działa, jesteś gotowy.
:::
